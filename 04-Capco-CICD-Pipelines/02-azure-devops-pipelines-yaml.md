...
"Used Azure DevOps to automate deployment" is the resume bullet's exact phrasing, and Azure DevOps
Pipelines is almost certainly the concrete tool the candidate used day-to-day. This chapter covers
the YAML pipeline model in enough depth to write one from scratch in an interview whiteboard/live-code
setting, and walks through a realistic pipeline for deploying a containerized FastAPI app — the shape
of both the chatbot backend (course 01) and the document-uploader service (course 03, built with
FastAPI, SQLAlchemy as the ORM, and RBAC via MSAL/OAuth 2.0 against Azure AD) — to Azure App Service.

## The YAML Object Model: Stages, Jobs, Steps

Azure Pipelines YAML has a strict nesting hierarchy, and understanding *why* it's nested this way
(not just the syntax) is what separates "I copy-pasted a pipeline" from "I understand what I'm
configuring":

- **Pipeline** — the whole file; the top-level unit triggered by an event (a push, a PR, a schedule).
- **Stage** — a major phase of the pipeline (e.g., `Build`, `DeployDev`, `DeployProd`). Stages run
  sequentially by default, but can run in parallel, and each stage can depend on and consume the
  outputs of a previous stage. Stages are the natural boundary for **approval gates** — you gate
  entry into a stage, not entry into a job or step.
- **Job** — a unit of work that runs on a single agent (a VM or container that executes your steps).
  ...
  declare dependencies between them. Jobs are the boundary for **agent pool selection** — e.g., a
  Linux container-build job vs. a Windows-specific job.
- **Step** — the smallest unit: a single script invocation or a pre-built **task** (Microsoft- or
  community-published, like `Docker@2` or `AzureWebApp@1`). Steps within a job run sequentially on
  the same agent and share filesystem state.
...
reason about where to put an approval gate (stage) versus where to put a parallel matrix build (job)
versus where to put a shell command (step).
...
YAML fragments parameterized like functions — specifically to avoid copy-pasting that shape:
...
of the YAML file, which matters a lot once you're managing multiple client subscriptions:
...
- **Variable groups** are named sets of key-value pairs (optionally backed by Azure Key Vault)
  ...
  rather than hardcoding connection strings or app names inline.
- **Service connections** are the authenticated bridge between Azure DevOps and an external system —
  ...
  container registry or GitHub. A service connection is scoped and permissioned once by an admin;
  pipelines reference it by name and never see the underlying credential.
...
For a multi-client consulting setup, this pairing — one variable group and one service connection per
client environment, referenced by parameterized templates — is the practical answer to "how do you
keep Client A's secrets from ever touching Client B's pipeline run."

## Triggers: CI, PR, and Scheduled

- **CI triggers** fire a pipeline run when commits land on specified branches (typically `main` after
  a PR merge). This is what drives the "every merge builds and deploys to dev" behavior.
  ```yaml
  trigger:
    branches:
      include: [main]
    paths:
      exclude: [docs/*]
  ```
- **PR triggers** fire a *validation* run (build + test, no deploy) when a pull request is opened or
  updated, without touching any real environment. This is the mechanism behind "tests must pass
  ...
  pr:
    branches:
      include: [main]
  ```
- **Scheduled triggers** fire on a cron-like schedule independent of code changes — useful for
  nightly full regression suites, dependency-vulnerability re-scans, or periodic infrastructure drift
  checks (`terraform plan` run nightly to confirm the real environment still matches the Terraform
  state).
  ```yaml
  schedules:
    - cron: '0 2 * * *'
...
(course 03: FastAPI, SQLAlchemy ORM, RBAC via MSAL/OAuth 2.0, Docker, Azure App Service):
...
Two framework-specific details worth calling out explicitly, since they're the only parts of this
pipeline that actually differ from a generic Python web service: `requirements.txt` (installed in the
`Install deps + lint` step) pulls in `fastapi`, `uvicorn[standard]`, `sqlalchemy`, and `msal` rather
than `flask` — those are the real dependencies of the document-uploader service (course 03) — and the
image built by the `Docker@2` step runs the app via `uvicorn` inside the container (see course 03's
Dockerfile `CMD` for the exact invocation), not `gunicorn`+Flask. The `Post-deploy smoke test` step
hits the service's FastAPI `/health` endpoint (`scripts/smoke_test.py` appends `/health` to the
`--url` value — see notebook 03) exactly the same way it would for any other framework; that's the
point made below about the pipeline being framework-agnostic.
...
is the one worth describing when an interviewer asks you to sketch a pipeline live: it's tool-neutral
in structure even though the syntax above is Azure DevOps-specific.
...
the same pipeline definition. Duplicating the whole `azure-pipelines.yml` per client would mean two
places to keep in sync every time the pipeline itself changed, which is exactly the maintenance
problem templates (above) exist to solve. The fix is a **client-parameterized deployment template**
plus a **stage matrix**, so the pipeline definition stays single while the *deployment* fans out per
client.

First, the per-client deploy logic becomes a parameterized template, same idea as
...
2. **The service connection and variable group are the actual isolation boundary.** The YAML
   ...
   subscription is that `hsbc-prod-service-connection` is scoped (via RBAC on the underlying service
   principal) to only HSBC's subscription/resource group, and `hsbc-prod-secrets` only ever contained
   HSBC's Key Vault URI and connection strings. Swapping the parameter values swaps the entire target
   environment — there's no code path where a client's data or credentials could leak into the
   other's pipeline run.
3. **Each client's `environment:` resource carries its own approval gate**, so HSBC's production
   sign-off and Bank of America's production sign-off are two separate, independently-configured
   Azure DevOps checks — one client's approver list has no visibility into or control over the other's
   deployment.
...
driven by a client list stored in a variable/parameters file) — the template and the Build stage
never change.
