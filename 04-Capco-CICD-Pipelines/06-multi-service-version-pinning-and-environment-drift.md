...
This chapter exists for the same reason course 03's chapter 05 exists: because there's a sharp,
...
**A note on how this chapter was built, unlike course 03's chapter 05:** course 03 was reconstructed
...
The honest answer, like course 03's, has five parts: what a realistic setup does today (three
...
model-risk-monitoring platform (course 02) that shows systems-level thinking rather than
CI/CD-tunnel-vision.

## Part 1 — What a realistic setup does today: three independent release trains, no shared manifest

The chatbot (course 01), the model-risk-monitoring platform (course 02), and the document-uploader
service (course 03) each have their **own** Azure DevOps pipeline (chapter 02), their own Docker image
...
gap here as course 03's chapter 05 is about the versioning gap: *"Today, each of the three services
...
2. **A cross-service version-check step**, added as a manual or scheduled (not release-blocking) stage
   ...
   direct analog of course 03 chapter 05's "search, remove, re-upload" manual workaround: **the pieces
...
**Caveat worth stating alongside this, precisely because it mirrors course 03's chapter 05 caveat:**
...
v41's Ragas-based groundedness scoring (course 02) was changed to read that field directly. Monitoring
...
This is the point worth volunteering to show systems-level thinking, the same way course 03's chapter 05
...
but the model-risk-monitoring platform (course 02) is the system that has to *interpret* metrics
...
harmfulness scoring from course 02) do two things it currently has no way to do if the version is only
...
questions are never being confused under pressure. As with course 03's chapter 05, confidence about
