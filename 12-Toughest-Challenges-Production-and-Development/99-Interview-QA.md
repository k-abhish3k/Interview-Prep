...
| "For a new/revised version of X, how are you handling that?" (asked cold, about any project) | 09 (the generic supersede pattern) — not a war story, the five-step on-the-spot answer shape, see Q A1 |

Pick **2–3** stories total for a given interview, not one per question — most interviewers only
ask one or two variants of this question, and over-preparing to recite all eight war-story chapters
in one sitting reads as rehearsed rather than reflective.

## Part A — Direct "toughest problem" questions (7+)

**A1. "Interviewers sometimes ask 'for a new/revised version of X, how are you handling that' about almost any data pipeline — how do you answer that on the spot for a project you haven't specifically prepared this angle for?"**
This is chapter 09's whole reason for existing, and it's worth leading with the honest framing chapter
09 itself opens with: this isn't a sign any of these nine projects was built carelessly — versioning
is almost never in the original ticket, because the need only becomes visible once a first revision
actually happens. With that framing set, walk the same fixed, five-step shape regardless of which
project or entity the interviewer names:

1. **Name what happens today, plainly.** For almost any of these pipelines, the honest default is:
   nothing automated — a new upload, a new detection run, a new extracted claim, a new generated
   section is treated as an independent unit with no awareness that an earlier version of the same
   logical thing exists.
2. **Name the manual workaround, if one exists.** Most of these systems already have the raw pieces
   ...
   "supersede" outcome by hand. The gap is usually that nothing automates or enforces the sequence,
   not that it's structurally impossible.
3. **Explicitly separate the two concerns that get conflated.** Almost every one of these systems has
   *some* status/lifecycle machinery already (an approval workflow, a monitoring alert state, a
   ...
   tracks whether *this one instance* is technically valid or approved, not whether it's still
   *current* relative to a newer instance. Naming that distinction unprompted is the single strongest
   signal in this answer.
4. **Propose a concrete mechanism.** The reusable shape is the same everywhere: a stable group/logical
   ...
   version to the one before or after it, and a well-defined "get current version" query — not an
   ambiguous re-search by name or content every time.
5. **Name the downstream system that also needs to know.** A version pointer added to one service's
   own database only solves half the problem — whatever consumes that entity downstream (a search
   index, a RAG retrieval layer, a dashboard, a compliance report) keeps serving or comparing against
   the stale version until it's told about the new one too.

The source of this pattern is worth citing directly if asked: it was a real, source-grounded finding
from rebuilding course 5 (Document Uploader Service) against the candidate's actual proprietary code —
no built-in document-versioning, only a manual search-remove-reupload workaround — and chapter 09
generalizes that same shape across all nine project courses. `notebooks/04_generic_supersede_pattern_demo.ipynb`
in this folder implements the reusable `id` / `version` / `supersedes_id` shape from step 4 directly,
runnable and independent of any one project's domain specifics.

*Likely follow-up 1: "Walk through this for a project I pick right now — say, the chart/graph detection course."*
Apply the same five steps live: today, a re-run on a resubmitted image produces a second, independent
detection result with nothing tying it to the first (step 1); the manual fix is finding and retiring
the prior result by hand (step 2); a detection's confidence/review status is a different axis from
whether it's the *current* result for that source image, and conflating them risks either duplicate
detections or silently dropping a genuinely revised one (step 3); tag each detection with a
`supersedes_detection_id` pointer and a stable `source_image_group_id` (step 4); and whatever
downstream reporting rolls detections into a client-facing count needs to stop double-counting the
superseded one, not just this pipeline's own database (step 5). The specific nouns change per
project; the shape doesn't.

*Likely follow-up 2: "Isn't this just restating 'add a version column' with extra steps?"*
No — and saying so explicitly is the stronger answer. "Add a version column" alone doesn't distinguish
between an entity that's stale and one that's merely superseded-but-still-historically-relevant, and it
says nothing about who else needs to be told. The reason this framework holds up under follow-up
questions is specifically steps 3 and 5: naming the conflated concern before being asked, and naming
the downstream consumer that also needs the new version — a plain version column answers "which one is
newest," not "who else is still trusting the old one."

**A2. "Tell me about the most challenging technical problem you've faced in your career."**
...
**A3. "Tell me about a difficult production incident. What happened, and what did you do?"**
...
**A4. "Tell me about a time you found a serious problem before it caused real damage."**
...
**A5. "Tell me about the hardest bug you've ever debugged."**
...
**A6. "Tell me about a time a project was harder than expected, before it even launched."**
...
**A7. "Tell me about a time you had to balance competing priorities under pressure."**
...
**A8. "What's a mistake you made, and what did you learn from it?"**
None of the eight war-story chapters is framed as "I personally made a mistake" — they're framed as
problems encountered and solved. If asked this directly, the honest move is to pick the chapter
closest to a real mistake you can speak to (e.g., a version of chapter 01 where the load test
*should* have happened earlier, or chapter 07 where the network review *should* have been scoped
during initial planning) and lead with the "what I'd do differently" angle from the start, rather
than trying to retrofit one of these into a pure external-failure story.
