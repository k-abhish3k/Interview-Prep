...
Course 4 (Model Risk Monitoring) builds exactly the harness this chapter depends on: LLM and
...
4. **Root-cause and fix** — often requires re-running the golden evaluation set (course 8) against
...
> **Illustrative scenario, not a verified incident record.** Built from the Course 4 monitoring
> harness and the Capco/Azure production stack described above.

**Situation.** A prompt-engineering change was deployed to a banking client's AI Assistant (course 1) intended to make responses more concise, based on internal feedback that answers were running
...
**Task.** As the engineer who also owned the Model Risk Monitoring pipeline (course 4) for this
...
supporting-detail sentences intact, validated against the golden evaluation set (course 8) before
...
- Continuous/sampled quality monitoring (course 4's harness, running in production) as the
