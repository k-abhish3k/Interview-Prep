...
## A worked example: `Document` model plus a FastAPI CRUD router
...
The resume bullet's "CRUD functionality" is satisfied by the path operations above; "end to end" is
satisfied by everything each path operation composes via `Depends()` — a database session (this
chapter), an authenticated identity and role check (chapter 06), and, upstream of all of it, Key
Vault-sourced configuration (chapter 05). FastAPI's dependency-injection system is what makes that
composition explicit and independently testable rather than tangled together inside each route
function — which is a stronger, more concrete answer to "how is this code organized?" than "it's a
