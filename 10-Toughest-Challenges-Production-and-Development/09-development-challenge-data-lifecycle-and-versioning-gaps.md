...
course 3 (Document Uploader Service) directly from the candidate's actual proprietary source code, a
...
up in detail, with the exact function and endpoint names, in course 3's
[`05-document-lifecycle-versioning-and-revisions.md`](../03-Capco-Document-Uploader-Service/05-document-lifecycle-versioning-and-revisions.md).

What makes that finding worth a whole chapter here, rather than staying a course-3 footnote, is that
...
|---|---|---|
| 3 — Document Uploader Service | The uploaded document itself | [`05-document-lifecycle-versioning-and-revisions.md`](../03-Capco-Document-Uploader-Service/05-document-lifecycle-versioning-and-revisions.md) — the source-grounded original finding |
...
| 2 — Model Risk Monitoring | The monitoring baseline a model's live metrics are compared against | [`06-baseline-staleness-and-drift-lifecycle.md`](../02-Capco-Model-Risk-Monitoring/06-baseline-staleness-and-drift-lifecycle.md) |
| 4 — CI/CD Pipelines | The pinned version of a shared service/dependency across environments | [`06-multi-service-version-pinning-and-environment-drift.md`](../04-Capco-CICD-Pipelines/06-multi-service-version-pinning-and-environment-drift.md) |
| 5 — Superscript/Subscript Detection | The processed document/page run through the OCR pipeline | [`06-document-reprocessing-and-citation-deduplication.md`](../05-Indegene-Superscript-Subscript-Detection/06-document-reprocessing-and-citation-deduplication.md) |
| 6 — GenAI Virtual Liaison Platform | The Pinecone vector for a source document chunk | [`08-vector-index-staleness-and-document-revision-handling.md`](../06-Indegene-GenAI-Virtual-Liaison-Platform/08-vector-index-staleness-and-document-revision-handling.md) |
| 7 — Claim Extraction & Tagging | The approved-claims library entry a new claim is checked against | [`07-approved-library-versioning-and-stale-comparison.md`](../07-Indegene-Claim-Extraction-Tagging/07-approved-library-versioning-and-stale-comparison.md) |
| 8 — Chart/Graph Detection | The chart/graph detection result for a given source image | [`06-detection-deduplication-and-model-version-drift.md`](../08-Indegene-Chart-Graph-Detection/06-detection-deduplication-and-model-version-drift.md) |
| 9 — GenAI Regulatory Platform | The clinical protocol / SOC document and its amendments | [`06-protocol-amendment-versioning-and-document-revision-handling.md`](../09-Indegene-GenAI-Regulatory-Platform/06-protocol-amendment-versioning-and-document-revision-handling.md) |
...
rehearsed this angle for, the course-3 answer (Part 5 of chapter 05 there) is the template. It has a
...
3. **Explicitly separate the two concerns that get conflated.** Almost every one of these systems has
   *some* status/lifecycle machinery already (an approval workflow, a monitoring alert state, a
   ...
   not whether it's still *current* or *true* relative to a newer instance. Course 3's IWPB approval
...
The value of internalizing this as a *pattern*, rather than memorizing course 3's specific answer, is
