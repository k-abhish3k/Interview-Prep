...
- **Multi-model endpoints** (hosting several models behind one endpoint, loaded on demand) can
  ...
  chart-detection models from courses 5 and 8) that individually don't justify a dedicated
  always-on endpoint.
...
feature (in the shape of course 9's regulatory-document generation platform) provisioned with a
