...
of both the chatbot backend (course 01) and the document-uploader service (course 05, built with
...
than `flask` — those are the real dependencies of the document-uploader service (course 05) — and the
image built by the `Docker@2` step runs the app via `uvicorn` inside the container (see course 05's
