...
Course 05 has a chapter built around a real interview question the candidate was actually asked:
...
model?* This chapter is that answer, built out with the same rigor as course 05's chapter 05.

**A note on how this chapter should be read, unlike course 05's:** course 05 was rebuilt from real
...
wired together for this purpose, exactly the "the pieces exist, nothing automates them" shape course 05's chapter 05 describes for its own versioning gap:
...
**Caveat worth stating alongside this, matching course 05's:** this manual process, even done
...
throughout, in the same spirit as course 05's `document_group_id`/`supersedes_document_id` proposal.
...
same way course 05's chapter 05 treats its own downstream retrieval layer.
...
Both failure modes share the same shape as course 05's retrieval tie-in: **the fix has to reach as far
...
course 05, being precise about what's *not* handled today, and exactly why two adjacent-sounding
