...
verified and what isn't**: unlike course 5 (rebuilt from an actual source repository with function
...
document-generation systems. It is written with the same confidence and specificity as course 5's
...
This is worth stating plainly as the honest gap, in the same register course 5's chapter 05 states
...
course 5's "the pieces exist, they're just not wired together automatically" framing — the fix
...
The parallel worth drawing explicitly, because it's the same mistake in a different course: course 5's chapter 05 warns against conflating IWPB's approval/expiry state machine with actual document
...
there is no source repository behind this platform the way there is for course 5 — so the honest
