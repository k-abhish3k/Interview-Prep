...
This service was built with **FastAPI**, an ASGI (Asynchronous Server Gateway Interface) web framework
built on top of **Starlette** (routing/request handling) and **Pydantic** (data validation). Three
things make it a strong fit for a document-ingest microservice specifically:
...
FastAPI maps a URL and HTTP method to a Python function via a decorator on an `APIRouter` (roughly the
...
Two differences from the Flask shape are worth naming explicitly in an interview:

1. **The path parameter's type is a plain Python type annotation**, not a URL converter syntax. FastAPI
   validates and coerces `doc_id: int` the same way it validates request bodies, and an invalid value
   produces a `422` automatically.
2. **Errors are raised as exceptions** (`HTTPException`), not returned as `(body, status_code)` tuples.
   FastAPI's exception handling middleware converts them into the right JSON response, which keeps a
   route's "happy path" return type clean — it only ever returns the success shape.
...
have defaults or constraints. FastAPI uses it in both directions:
...
FastAPI returns `422` with a structured error listing exactly which field failed and why.

On the way out, `response_model=DocumentOut` filters and validates the return value against that shape.
So even if the underlying SQLAlchemy object has extra internal fields (an `idempotency_key`, an ORM
...
which is exactly the right place to close the session whether the request succeeded or raised.

This same mechanism is what chapter 06 uses for authentication and RBAC:
`current_user: User = Depends(get_current_user)` and
`_: None = Depends(require_role("DocumentUploader.Write"))` are the same pattern applied to "who is
calling" and "are they allowed to do this" — composed independently of the database-session dependency,
and of each other.
...
- **Maintainability.** A `Document` model declared once, in one place, with typed columns, is the single
  source of truth for the table's shape. Every place in the codebase that touches a document works
  against that model, instead of every hand-written query independently getting the column list and
  types right (and silently drifting out of sync with the actual schema over time).
...
chapter 05 (including `tenant_id`, soft-delete, and audit columns). The ORM doesn't replace that schema
design — it's the Python-side representation of it.
...
table, SQLAlchemy would express the one-to-many relationship declaratively, rather than requiring a
manual join in every query that needs both:
...
`doc.events` then loads the related rows as Python objects — lazily, or eagerly with
`selectinload`/`joinedload` when you want to avoid N+1 queries. The join is generated by the ORM, not
hand-written at every call site that needs it.

## The real models: `ApproverMapping` and `IWPBDocumentWorkflow`

This is where the real codebase (`src/uploader/tables/uploader_table.py`,
`src/uploader/tables/base.py`) departs from a textbook `Document` model. Two things are worth naming
immediately.
...
**2. The two real tables use the older `sqla.Column(...)` declarative style, rather than the typed
`Mapped[...]`/`mapped_column(...)` style shown earlier in this chapter.** Both styles are valid
SQLAlchemy — this codebase predates a migration to the newer style:
...
`checkfirst=True` means "create any table that doesn't already exist; leave existing tables alone."
This is genuinely convenient — a fresh environment gets its schema for free on first boot, with **no
manual migration step, and no Alembic** anywhere in this codebase.

Here's the real, honest gap this leaves: `create_all` only ever *adds* new tables. It has no concept of
altering an existing table. Add a column to `IWPBDocumentWorkflow` in code, and `create_all` silently
does nothing about the already-existing database table on the next boot — because the table already
exists, and `checkfirst` skips it entirely.
...
dependency injection, response models) in the more familiar shape of a classic CRUD resource. The real
`IWPBDocumentWorkflow` model above is entangled with the approver-workflow logic covered in full in
chapter 05, so this example exists purely to make the mechanics easy to internalize.
...
`notebooks/01_fastapi_crud_api_demo.ipynb`).

Note this teaching example uses a generic SaaS `tenant_id` column to demonstrate row-scoping. That's a
standard multi-tenant CRUD pattern worth knowing in general — but it is **not** this real service's
isolation axis. This service's real "who can see what" boundary is **business_line/use_case** (IWPB vs.
FEMA vs. TPMB vs. GTRM vs. generic), enforced via the feature-flag-gated RBAC in chapter 04 and the
department routing in chapter 03, not a `tenant_id` column. There's no multi-bank or multi-client
tenancy in this specific service's real design:
...
business logic, and the `tenant_id` filter present on every single query. That's exactly the separation
of concerns an interviewer is listening for when they ask "how would you test this without a real
database, and how do you know two tenants can't see each other's documents?"
...
more concrete answer to "how is this code organized?" than "it's a Flask app with some helper
functions."
