...
questions** — the same positioning course 3's Q1 gets for its document-versioning question, because
...
- *"Would you call this a gap in the system, or something that was never in scope?"* — Worth answering
  the way course 3's equivalent question is answered: honestly, as a gap that's a natural consequence
...
- **Vector versioning built in from day one** (Chapter 8) — rather than adding `document_version` /
  ...
  course 3 would now add `document_group_id` at the source-document layer from day one.
