...
Course 3's document-uploader rebuild has a chapter answering a real interview question the
...
**An important honesty note before going further, unlike course 3's chapter 05**: there is no real
source repository backing this course. Course 3 was rebuilt from actual proprietary code the
...
The chapter has the same five-part shape as course 3's chapter 05: what a realistic implementation
...
gap course 3 names for document versioning — not "we don't support that," but "the pieces to check
...
As with course 3's uploader, nothing about this gap is unfixable *by hand* — it's just entirely
...
course 3's chapter 05 makes about IWPB's approval state machine not being the same axis as document
...
- **Perceptual hash match within a tolerance, but not an exact match** (`supersede`) is the Concern-B
  ...
  the new ones. This mirrors course 3's `supersedes_document_id` proposal directly, adapted to a
...
something stale, exactly the kind of proportionate design course 3's chapter 06 argues for elsewhere
in this curriculum.
...
know which of the two is current. Exactly as course 3's chapter 05 argues for a RAG index sitting
...
the distinction is understood, not just gestured at. As with course 3's equivalent chapter, being
